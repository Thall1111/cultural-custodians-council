...
(function(){
  ...
  function regen(){
    ...
    ys.forEach((y,i)=>{
      const p=mk(colors[i]);
      p.setAttribute('d', build(w,h,7,y,amps[i],h*.06));
      svg.appendChild(p);
    });
  }
...
(function(){
  ...
  function updateLogos(theme){
    if(theme==='darkblue'){
      if(navbarLogo) navbarLogo.src='CCC_Logo_Light.png';
      if(heroLogo) heroLogo.src='CCC_Logo_Dark.png'; // keep hero dark
    } else {
      if(navbarLogo) navbarLogo.src='CCC_Logo_Dark.png';
      if(heroLogo) heroLogo.src='CCC_Logo_Dark.png';
    }
  }
